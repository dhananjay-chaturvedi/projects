...
from common.config_loader import console_debug
...
def connectSQLite(database, **kwargs):
    ...
    try:
        ...
        set_autocommit(conn, "SQLite", default_autocommit())
...
def pingSQLite(conn):
    """Return True when the SQLite connection is alive."""
    if not conn:
        return False
    cursor = None
    try:
...
def reconnectSQLite(conn, database, **kwargs):
    """Reconnect to SQLite using the database file path."""
    try:
        disconnectSQLite(conn)
    except Exception:
        pass
    return connectSQLite(database=database, **kwargs)
...
def getSQLiteTableSchema(conn, table_name):
    ...
    try:
        ...
        columns = []
        for row in rows:
            _, col_name, col_type, notnull, default_val, is_pk = row
