...
def log(message):
    """Driver INFO trace.  Goes to the file logger; only echoes to stdout
    when ``[logging] enable_info`` is true (off by default for production)."""
...
def connectMariadb(params=None, **kwargs):
    ...
    try:
        ...
        row = cursor.fetchone()
        current_db = row[0] if row else None  # type: ignore[index]
        cursor.close()
        ...
        if current_db:
            ...
            log(
                f"Connected to MariaDB server at {host}:{port} but no database selected"
            )
...
def pingMariadb(conn):
    """Return True when the MariaDB connection is alive."""
    try:
        if not conn:
            return False
...
def reconnectMariadb(conn, params=None, **kwargs):
    """Reconnect to MariaDB using the supplied connection parameters."""
    try:
...
def isRoot(conn):
    ...
    try:
        ...
        for grant in grants:
            ...
            if (
                "ALL PRIVILEGES" in grant_text
                or "SUPER" in grant_text
                or "root@" in current_user
            ):
                return True
        return False
    except Error:
...
def getMariadbConstraints(conn):
    ...
    try:
        ...
        """)
        constraints = [
            f"{decode_value(row[0])}.{decode_value(row[1])} ({decode_value(row[2])})"
            for row in cursor.fetchall()
        ]
        cursor.close()
        return constraints
...
def getMariadbCharsets(conn):
    ...
    try:
        ...
        charsets = [
            f"{decode_value(row[0])} - {decode_value(row[2])}"
            for row in cursor.fetchall()
        ]
        cursor.close()
        return charsets
...
def getMariadbProcessList(conn):
    ...
    try:
        ...
        processes = [
            f"ID:{row[0]} User:{decode_value(row[1])} DB:{decode_value(row[3]) if row[3] else 'None'} Command:{decode_value(row[4])}"
            for row in cursor.fetchall()
        ]
        cursor.close()
        return processes
