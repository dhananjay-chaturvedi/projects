...
from common.drivers.ssl_support import SslParams, mysql_ssl_connect_kwargs
...
def connectMysql(params=None, **kwargs):  # Connect to MySQL database
    """
    ...
    autocommit_default = config.get_bool(
        "database.connection", "default_autocommit", default=False
    )

    # mysql.connector uses connection_timeout for initial TCP handshake.
    ...
    try:
        ...
        if connect_timeout and int(connect_timeout) > 0:
            connect_kwargs["connection_timeout"] = int(connect_timeout)
        charset = kwargs.get("charset")
        if charset:
            from common.charset_util import mysql_charset_name

            connect_kwargs["charset"] = mysql_charset_name(charset)
...
def pingMysql(conn):
    """Return True when the MySQL connection is alive."""
    return validate_connection(conn)


def reconnectMysql(conn, params=None, **kwargs):
    """Reconnect to MySQL using the supplied connection parameters."""
    try:
...
def getMysqlTables(conn, database=None):  # Get MySQL database tables
    try:
        ...
        cursor = get_cursor(conn)
        if database:
            cursor.execute(f"SHOW TABLES FROM {_quote_identifier(database)}")
        else:
            # Get current database if not specified
            cursor.execute("SELECT DATABASE()")
            current_db = cursor.fetchone()
            if current_db and current_db[0]:
                cursor.execute(f"SHOW TABLES FROM {_quote_identifier(current_db[0])}")
            else:
                # No database selected, query information_schema for all databases
                cursor.execute(
                    "SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA NOT IN ('information_schema', 'mysql', 'performance_schema', 'sys') AND TABLE_TYPE = 'BASE TABLE' ORDER BY TABLE_SCHEMA, TABLE_NAME"
                )
...
def getMysqlIndexes(
    conn, table_name, database=None
):  # Get MySQL indexes for a specific table
    try:
        cursor = get_cursor(conn)
        if database:
            cursor.execute(
                f"SHOW INDEX FROM {_quote_identifier(database)}."
                f"{_quote_identifier(table_name)}"
            )
        else:
            cursor.execute(f"SHOW INDEX FROM {_quote_identifier(table_name)}")
        indexes = list(
            set([decode_value(row[2]) for row in cursor.fetchall()])
...
def getMysqlAllIndexes(conn, database=None):  # Get all indexes in database
    try:
        ...
        if database:
            ...
            """)
        indexes = [
            f"{decode_value(row[1])}.{decode_value(row[0])}"
...
def getMysqlUsers(conn):  # Get MySQL users
    try:
        ...
        users = [
            f"{decode_value(row[0])}@{decode_value(row[1])}"
            for row in cursor.fetchall()
        ]
        cursor.close()
        return users
...
def getMysqlConstraints(conn, database=None):  # Get MySQL constraints
    try:
        ...
        if database:
            ...
            """)
        constraints = [
            f"{decode_value(row[1])}.{decode_value(row[0])} ({decode_value(row[2])})"
            for row in cursor.fetchall()
        ]
        cursor.close()
        return constraints
...
def getMysqlTableColumns(
    ...
    try:
        ...
        if database:
            cursor.execute(
                f"SHOW COLUMNS FROM {_quote_identifier(database)}."
                f"{_quote_identifier(table_name)}"
            )
        else:
...
def getMysqlVariables(conn, pattern=None):  # Get MySQL system variables
    try:
        cursor = get_cursor(conn)
        if pattern:
            cursor.execute("SHOW VARIABLES LIKE %s", (str(pattern),))
        else:
...
def getMysqlStatus(conn, pattern=None):  # Get MySQL status variables
    try:
        cursor = get_cursor(conn)
        if pattern:
            cursor.execute("SHOW STATUS LIKE %s", (str(pattern),))
        else:
...
def getMysqlProcessList(conn):  # Get MySQL process list
    try:
        ...
        for row in cursor.fetchall():
            processes.append(
                f"ID:{decode_value(row[0])} User:{decode_value(row[1])} Host:{decode_value(row[2])} DB:{decode_value(row[3])} Command:{decode_value(row[4])} Time:{decode_value(row[5])}"
            )
        cursor.close()
        return processes
    except Error as e:
