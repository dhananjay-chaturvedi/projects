...
def install_database_drivers():
    ...
    # Check for Oracle driver (oracledb preferred, cx_Oracle fallback)
    print_info("\nChecking for Oracle driver...")
    try:
        from common.drivers import oracle_driver

        print_success(f"Oracle driver available ({oracle_driver.driver_label()})")
    except ImportError:
        print_warning("Oracle driver not installed")
        print_info("  Recommended: pip install oracledb")
        print_info("  Legacy fallback: pip install cx_Oracle --no-build-isolation")
        print_info(
            "  For Oracle 11g or Thick-only features, also install Instant Client: "
    ...
    try:
        ...
        print_info(
            "  May require system packages: brew install mariadb-connector-c (macOS)"
        )
...
def check_system_dependencies():
    ...
    if shutil.which("sshpass"):
        ...
        if sys.platform == "darwin":
            print_info(
                "  To install on macOS: brew install hudochenkov/sshpass/sshpass"
            )
...
def create_config_files():
    ...
    if not config_file.exists() and config_example.exists():
        ...
        print_warning(
            "⚠ Please edit config.ini with your database credentials and paths"
        )
...
def create_data_directories():
    """Create necessary data directories and run layout migration."""
    ...
    try:
        ...
        print_warning(f"Could not load layout module: {exc}")
        return False
    ...
    if result.get("ok"):
        ...
        if status == "migrated":
            print_success(f"Migrated legacy layout into {home}")
        elif status == "fresh":
            print_success(f"Initialised fresh layout at {home}")
        elif status == "current":
            print_success(f"Layout already current at {home}")
        else:
            print_success(f"Layout ready at {home} ({status})")
    else:
        print_warning(
            f"Layout migration reported issue: {result.get('message')}"
        )

    print_info(f"Data directory location: {home}")
    print_info("  This stores:")
    print_info("    - keys/       (db.key, cloud.key, monitor.key)")
    print_info("    - connections/ (db.json, cloud.json, monitor.json)")
    print_info("    - runtime/    (daemon.log, metrics.json, ...)")
    print_info("    - session/    (ai_state.json, dashboard_layout.json, ...)")
...
def print_next_steps():
    ...
    print(f"   - Install Oracle driver: pip install oracledb")
    ...
    # print(f"   - Install sshpass for SSH Monitoring")
    ...
    print(f"   All database credentials are encrypted and stored in ~/.dbassistant/")
